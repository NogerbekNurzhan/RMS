...
class RegistrationManager(models.Manager):
    ...
    def activate_user(self, activation_key, get_profile=False):
        ...
        If the key is valid but the ``User`` is already active, return ``User``.

        If the key is valid but the ``User`` is inactive, return ``False``.

        To prevent reactivation of an account which has been  deactivated by site administrators, ``RegistrationProfile.activated`` is set to ``True`` after successful activation.
        """
        ...
        if SHA1_RE.search(activation_key):
            try:
                ...
                # This is an actual activation failure as the activation key does not exist. It is *not* the scenario where an already activated User reuses an activation key.
                return False

            if profile.activated:
                # The User has already activated and is trying to activate again.
                # If the User is active, return the User.
                # Else, return False as the User has been deactivated by a site administrator.
    ...
    def create_profile(self, user, **profile_info):
        ...
        The activation key for the ``RegistrationProfile`` will be a SHA1 hash, generated from a secure random string.
        """
    ...
    def delete_expired_users(self):
        ...
        Accounts to be deleted are identified by searching for instances of ``RegistrationProfile`` with expired activation keys, and then checking to see if their associated ``User`` instances have the field ``is_active`` set to ``False``;
        ...
        1. It alleviates the ocasional need to reset a ``RegistrationProfile`` and/or re-send an activation email when a user does not receive or does not act upon the initial activation email; since the account will be deleted, the user will be able to simply re-register and receive a new activation key.

        2. It prevents the possibility of a malicious user registering one or more accounts and never activating them (thus denying the use of those usernames to anyone else); since those accounts will be deleted, the usernames will become available for use again.
...
class RegistrationProfile(models.Model):
    """
    A simple profile which stores an activation key for use during user account account.
    ...
    While it is possible to use this model as the value of the ``AUTH_PROFILE_MODULE`` setting, it's not recommended that you do so. This model's sole purpose is to store data temporarily during account account and activation.
    """
    ...
    def activation_key_expired(self):
        ...
        1. If the user has already activated, ``self.activated`` will be ``True``. Re-activating is not permitted, and so this method returns ``True`` in this case.

        2. Otherwise, the date the user signed up is incremented by the number of days specified in the setting ``ACCOUNT_ACTIVATION_DAYS`` (which should be the number of days after signup during which a user is allowed to activate their account); if the result is less than or equal to the current date, the key has expired and this method returns ``True``.
        """
    ...
    def send_activation_email(self, site, request=None):
        ...
        The activation email will use the following templates, which can be overriden by setting ACTIVATION_EMAIL_SUBJECT, ACTIVATION_EMAIL_BODY, and ACTIVATION_EMAIL_HTML appropriately:

        ``account/activation_email_subject.txt``
            This template will be used for the subject line of the email.
            Because it is used as the subject line of an email, this template's output **must** be only a single line of text; output longer than one line will be forcibly joined into only a single line.
        ...
        ``request``
            ...
            If supplied will be passed to the template for better  flexibility via ``RequestContext``.
        """
...
class SupervisedRegistrationManager(RegistrationManager):
    ...
    def admin_approve_user(self, profile_id, site, get_profile=False, request=None):
        """
        Approve the ``SupervisedRegistrationProfile`` object with the given ``profile_id``.

        If the id is valid, return the ``User`` after approving.

        If the id is not valid, return ``False``.

        If the id is valid but the ``User`` is already active, return ``User``.

        If the id is valid but the ``SupervisedRegistrationProfile`` object is not activated, return ``False``.
        """
        try:
            ...
            # If the user has not activated their profile the admin should not be able to approve his account (at least not following this process)
            if profile.activated:
    ...
    def send_admin_approve_email(self, user, site, request=None):
        """
        Send an approval email to the site administrators to approve this user.

        The approval email will use the following templates,  which can be overriden by setting APPROVAL_EMAIL_SUBJECT, APPROVAL_EMAIL_BODY, and APPROVAL_EMAIL_HTML appropriately:

        ``account/admin_approval_email_subject.txt``
            This template will be used for the subject line of the email.
            Because it is used as the subject line of an email, this template's output **must** be only a single line of text;
            Output longer than one line will be forcibly joined into only a single line.

        ``account/admin_approval_email.txt``
            This template will be used for the text body of the email.

        ``account/admin_approval_email.html``
            This template will be used for the html body of the email.
        ...
        ``profile_id``
            The id of the associated``SupervisedRegistrationProfile`` object.
        ...
        if not admins:
            raise ImproperlyConfigured(
                'Using the admin_approval account backend'
...
class SupervisedRegistrationProfile(RegistrationProfile):
    ...
    # Manager to implement the extra functionality required in admin approval.
    objects = SupervisedRegistrationManager()

    def send_admin_approve_complete_email(self, site, request=None):
        """
        Send an "approval is complete" email to the user associated with this ``SupervisedRegistrationProfile``.

        The email will use the following templates, which can be overriden by settings APPROVAL_COMPLETE_EMAIL_SUBJECT, APPROVAL_COMPLETE_EMAIL_BODY, and APPROVAL_COMPLETE_EMAIL_HTML appropriately:

        ``account/admin_approval_complete_email_subject.txt``
            This template will be used for the subject line of the email.
            Because it is used as the subject line of an email, this template's output **must** be only a single line of text; output longer than one line will be forcibly joined into only a single line.

        ``account/admin_approval_complete_email.txt``
            This template will be used for the text body of the email.

        ``account/admin_approval_complete_email.html``
            This template will be used for the text body of the email.
